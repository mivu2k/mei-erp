...
mkdir -p "$deploy_root/shared/logs" "$deploy_root/shared/app-data" "$deploy_root/shared/backups"
ln -sfn "$deploy_root/shared/logs" "$release/logs"
ln -sfn "$deploy_root/shared/app-data" "$release/App_Data"
ln -sfn "$deploy_root/shared/backups" "$release/backups"
...
if command -v systemctl >/dev/null && systemctl cat "$service" >/dev/null 2>&1; then
 sudo -n systemctl restart "$service"; for _ in {1..120}; do curl -fsS http://127.0.0.1:5090/health/ready >/dev/null && exit 0; sleep 1; done
