...
public static class RecordPrintEndpoints
{
    public static void MapRecordPrintEndpoints(this WebApplication app)
    {
        ...
        group.MapGet("/repair/job/{id:int}", async (int id,string? kind,string? size,int? template,IRepairService service,IPrintService print,ICompanyProfileService company,ILabelTemplateService templates,ICurrentUser user,CancellationToken ct)=>
        {
            ...
            var doc=new PrintDocument{Title=label?"DEVICE LABEL":delivery?"DELIVERY NOTE":"REPAIR JOB CARD",Reference=job.Number,Date=job.ReceivedOn,Size=label?PageSize.Label62:delivery?Size(size):PageSize.A4,Watermark=job.Status==JobStatus.Cancelled?"CANCELLED":null,
            ...
            return await Pdf(print,company,doc,$"{job.Number}-{(kind??"job-card")}{(delivery&&string.Equals(size,"roll",StringComparison.OrdinalIgnoreCase)?"-pos":"")}",ct);
        });
        ...
        group.MapGet("/inventory/return/{id:int}",async(int id,string? size,IInventoryReturnService service,IPrintService print,ICompanyProfileService company,ICurrentUser user,CancellationToken ct)=>
        {if(!user.Can(InventoryModule.ReturnsPost))return Results.Forbid();var r=await service.GetAsync(id,ct);if(r is null)return Results.NotFound();var doc=new PrintDocument{Title=r.Kind==InventoryReturnKind.SalesReturn?"CUSTOMER RETURN NOTE":"SUPPLIER RETURN NOTE",Reference=r.Number,Date=r.Date,Size=Size(size),Fields=[new("Party",r.PartyName),new("Original document",r.SourceReference),new("Reason",r.Reason),new("Posted by",r.PostedByName)],Tables=[Table([("Item",3f,false),("Qty",1f,true),("Unit cost",1.3f,true),("Serials / batch",2.5f,false)],r.Lines.Select(x=>(IReadOnlyList<string>)[x.ItemCode+" — "+x.ItemName,x.Quantity.ToString("0.##"),x.UnitCost.ToString("N2"),string.Join(" · ",new[]{x.SerialNumbers,x.BatchNumber}.Where(y=>!string.IsNullOrWhiteSpace(y)))]))],Totals=[new("Value",r.Total.ToString("N2"))],Notes=r.Notes,Signatures=["Posted by",r.Kind==InventoryReturnKind.SalesReturn?"Customer":"Supplier","Storekeeper"]};return await Pdf(print,company,doc,$"{r.Number}-{(string.Equals(size,"roll",StringComparison.OrdinalIgnoreCase)?"pos":"a4")}",ct);});
        ...
        group.MapGet("/sales/delivery/{id:int}",async(int id,string? size,TradeDbContext db,IPrintService print,ICompanyProfileService company,ICurrentUser user,CancellationToken ct)=>
        {if(!user.Can(SalesModule.OrdersView))return Results.Forbid();var d=await db.Deliveries.AsNoTracking().Include(x=>x.Lines).FirstOrDefaultAsync(x=>x.Id==id,ct);if(d is null)return Results.NotFound();var doc=new PrintDocument{Title="DELIVERY NOTE",Reference=d.Number,Date=d.Date,Size=Size(size),Fields=[new("Customer",d.PartyName),new("Collected by",d.CollectedBy),new("Sales order",d.SalesOrderId.ToString())],Tables=[Table([("Item",4f,false),("Quantity",1.5f,true)],d.Lines.Select(l=>(IReadOnlyList<string>)[l.ItemName,l.Quantity.ToString("0.##")]))],Notes=d.Notes,Signatures=["Delivered by","Collected by"]};return await Pdf(print,company,doc,$"{d.Number}-{(string.Equals(size,"roll",StringComparison.OrdinalIgnoreCase)?"pos":"a4")}",ct);});
        ...
        {if(!user.Can(TenderModule.FilesView))return Results.Forbid();var f=await files.GetAsync(id,ct);if(f is null)return Results.NotFound();var doc=new PrintDocument{Title="FILE MOVEMENT REGISTER",Reference=f.FileNumber,Date=f.OpenedOn,Fields=[new("Owner",f.OwnerReference+" — "+f.OwnerTitle),new("Status",f.Status.ToString()),new("Current holder",f.HolderName),new("Location",f.Location),new("Volume / rack",f.VolumeNumber)],Tables=[Table([("Date",1.2f,false),("Action",1.3f,false),("From",2f,false),("To",2f,false),("Due back",1.3f,false),("Purpose / remarks",3f,false),("Recorded by",1.6f,false)],f.Movements.OrderBy(x=>x.MovedOn).ThenBy(x=>x.Id).Select(x=>(IReadOnlyList<string>)[x.MovedOn.ToString("d MMM yyyy"),x.Action.ToString(),x.FromHolderName??x.FromLocation??"—",x.ToHolderName??x.ToLocation??"—",x.DueBack?.ToString("d MMM yyyy")??"",string.Join(" · ",new[]{x.Purpose,x.Remarks}.Where(v=>!string.IsNullOrWhiteSpace(v))),x.RecordedByName??""]))],Signatures=["Records clerk","Verified by"]};return await Pdf(print,company,doc,$"{f.FileNumber}-movements",ct);});
